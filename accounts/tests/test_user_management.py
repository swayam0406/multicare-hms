...
class UserListFilterTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        User.objects.create_user(
            username="alice_doc", email="ad@t.local",
            password="pass1234", role=User.Role.DOCTOR,
            first_name="Alice", last_name="Doctor",
        )
        User.objects.create_user(
            username="bob_nurse", email="bn@t.local",
            password="pass1234", role=User.Role.NURSE,
            first_name="Bob", last_name="Nurse",
        )
        User.objects.create_user(
            username="carol_recep", email="cr@t.local",
            password="pass1234", role=User.Role.RECEPTIONIST,
            first_name="Carol", last_name="Recep",
        )
...
class UserCreateAccessTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.admin = User.objects.create_user(
            username="uc_admin", email="uca@t.local",
            password="pass1234", role=User.Role.ADMIN,
        )
        cls.doc = User.objects.create_user(
            username="uc_doc", email="ucd@t.local",
            password="pass1234", role=User.Role.DOCTOR,
        )
...
class UserCreateFormTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.admin = User.objects.create_user(
            username="cf_admin", email="cfa@t.local",
            password="pass1234", role=User.Role.ADMIN,
        )
    ...
    def test_duplicate_username_rejected(self):
        User.objects.create_user(
            username="taken", email="t@t.local",
            password="pass1234", role=User.Role.PATIENT,
        )
        self.client.login(username="cf_admin", password="pass1234")
        response = self.client.post(reverse("accounts:user_create"), {
    ...
    def test_duplicate_email_rejected(self):
        User.objects.create_user(
            username="original", email="dup@t.local",
            password="pass1234", role=User.Role.PATIENT,
        )
        self.client.login(username="cf_admin", password="pass1234")
        response = self.client.post(reverse("accounts:user_create"), {
